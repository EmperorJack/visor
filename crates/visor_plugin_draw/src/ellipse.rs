...
pub(crate) fn build_ellipse(draw: &Draw, commands: Vec<ShapeCommand>) {
    let _ellipse =
        commands
            .into_iter()
            .fold(draw.inner.ellipse(), |drawing, command| match command {
                ...
                _ => panic!("Unexpected: invalid shape command for ellipse"),
            });
}
...
pub(crate) fn op_draw_ellipse(state: &mut OpState, draw_id: u32) -> u32 {
    ...
    sketch_state
        .start_drawing_shape(ShapeKind::Ellipse, DrawId(draw_id))
        .0
}
